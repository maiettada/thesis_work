...
Nella tabella scores-comparisons.pdf \ref{fig:v1-nums} riportiamo i valori di Precision, Recall e Accuracy ottenuti per ogni entità contemplata nel task di classificazione, sia nel caso di NER\_regex che nel caso NER\_ML.
...
Si può facilmente notare come molte entità di tipo \textbf{Categoria} vedono valori di precision recall e accuracy migliori nell'approccio Machine Learning; in particolare sono stati evidenziati in verde i valori di precision, recall ed f1-score che in NER\_ML superano almeno dello 0.05 i corrispondenti valori registrati dall'NER\_regex. 
...
\acrshort{named-entity-recognition}. Chiameremo \(NER\_HITL\sb{n}\) il task di NER condotto con la  \(NN\sb{n}\) così prodotta
a valle del loop.
...
\[\Delta(r)\sb{i=10}=(34\%)\]
\[\Delta(f)\sb{i=10}=(32\%)\]
%È interessante notare come alla settima iterazione correttiva tali guadagni ammontano già al \[\Delta\sb{i=7}(P,R,F1)=(21\%,33\%, 31\%)\]
...
Va considerato però il tempo di produzione delle annotazioni gold e \(silver\sb{corrected}\):
...
risulta che abbiamo risparmiato
 ...
 pari a più di 6 ore per la produzione del dataset.
...
\paragraph{HITL-14}
Se la precision \(p\sb{10}\) del sistema \(NER\_HITL\sb{10}\) è soddisfacente, abbiamo mostrato una \(r\sb{10}\) che 
...
Per raggiungere l'iterazione 14 è stato necessario fornire \(#annot-HITL\sb{10}=2000 s\) annotazioni \(silver\sb{corrected}\).
...
È da notare l'iterazione HITL-14 che, adoperando un numero di annotazioni pari a\(n\sb{annotHITL14}=2000\) maggiore rispetto al caso \(i=10\), ottiene delle performance superiori al NER\_ML richiedendo un tempo d'annotazione molto minore rispetto al
NER\_ML stesso.
