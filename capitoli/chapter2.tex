...
La ricerca di attestazioni SOA in un testo è un problema di \textbf{Named Entity Recognition} (\textbf{NER}).
...
Più in generale, per ogni attestazione SOA che il sistema individuerà in un documento, dovranno essere mandate in output le seguenti informazioni:
\begin{itemize}
...
abbia senso operare. Solitamente i modelli NER contemplano entità come
...
Questo ci impone di contemplare nel \textbf{dominio} non solo l'entità, ma anche il tipo, che 
...
Ogni output del sistema può essere valutato in termini di TP-FP-TN-FN; disponendo del numero di TP-FP-TN-FN è
...
L'accuratezza di un sistema è spesso calcolata con la \textit{F1-score}, costituita dalla media armonica di 
Precision e Recall:
...
Disponendo di una ground truth, potremo procedere a valutare l'output del sistema e quindi classificare ogni entità in output come true positive, false positive, true negative, false negative.
